...
FOOD_KEYWORDS = ["hill's", "hills", "royal canin", "purina", "proplan", "iams", "eukanuba", "orijen", "acana", "farmina", "vetlife", "wellness",
                 "taste of the wild", "nutro", "pouch", "tin", "can", "canned", "wet", "dry", "kibble", "tuna", "chicken", "beef", "salmon",
                 "lamb", "duck", "senior", "diet", "food", "grain", "rc"]
XRAY_KEYWORDS = ["xray", "x-ray", "radiograph", "radiology"]
ULTRASOUND_KEYWORDS = ["ultrasound", "echo", "afast", "tfast", "a-fast", "t-fast"]
LABWORK_KEYWORDS = ["cbc", "blood test", "lab", "biochemistry", "haematology", "urinalysis", "idexx", "chem", "cytology", "smear", "faecal",
                    "fecal", "pcr", "fiv", "felv", "microscopic", "slide", "bun", "crea", "phos", "urine", "cpl", "cpli", "lipase", "amylase",
                    "pancreatic", "cortisol"]
...
# ----------------------------------------------------------
# STEP 1: Precompute and Cache All Transaction Blocks
# ----------------------------------------------------------
@st.cache_data(show_spinner=False)
def prepare_factoids_data(df):
    """Preprocess and cache all block-level and monthly data for reuse."""
    df = df.copy()
...
# ----------------------------------------------------------
# STEP 2: Compute Monthly Percentages for Each Metric
# ----------------------------------------------------------
@st.cache_data(show_spinner=False)
def compute_monthly_data(df_blocked, tx, patients_per_month, rx_pattern, apply_amount_filter=False):
    ...
    if isinstance(rx_pattern, re.Pattern):
        ...
        mask = df_blocked["Item Name"].str.contains(rx_pattern, na=False, case=False)
...
# STEP 3: Render Chart
# ----------------------------------------------------------
def render_chart(df_blocked, tx, patients_per_month, metric_configs):
    ...
    monthly = compute_monthly_data(df_blocked, tx, patients_per_month, config["rx"], config.get("filter", False))
    if monthly.empty:
        st.info(f"No qualifying {metric_choice.lower()} data found.")
        return

    chart = (
        alt.Chart(monthly)
        .mark_bar(size=35, color=config["color"])
        .encode(
            x=alt.X("MonthLabel:N", sort=monthly["MonthLabel"].tolist(),
                    axis=alt.Axis(title=None, labelAngle=45, labelFontSize=12)),
            y=alt.Y("Percent:Q", title=f"% of Monthly Patients Having {metric_choice}",
                    axis=alt.Axis(format=".1%")),
            tooltip=[
                alt.Tooltip("MonthLabel:N", title="Month"),
                alt.Tooltip("UniquePatients:Q", title=f"{metric_choice} Patients", format=",.0f"),
...
def run_factoids():
    ...
    # Precompute (cached)
    if "factoids_cache" not in st.session_state:
        df_blocked, tx, patients_per_month = prepare_factoids_data(df)
        st.session_state["factoids_cache"] = (df_blocked, tx, patients_per_month)
    else:
        df_blocked, tx, patients_per_month = st.session_state["factoids_cache"]
    ...
    render_chart(df_blocked, tx, patients_per_month, metric_configs)
