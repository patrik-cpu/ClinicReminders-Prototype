...
class VisualThemeCssTests(unittest.TestCase):
    ...
    def test_outcome_success_window_label_matches_due_date_logic(self):
        ...
        self.assertIn("In Track, this also counts a reminder as successful when a matching sale happens within this many days after the reminder is sent.", source)
    ...
    def test_stats_page_folds_outcomes_and_actioning_tabs(self):
        ...
        self.assertIn('MAIN_SECTION_TABS = ["Reminders", "Search Terms", "Exclusions", "Identify", "Stats", "Upload Data", "Get Started"]', source)
        self.assertNotIn('"Graphs": "graphs"', source)
        self.assertIn('"Reminders": "Send Reminders"', source)
        self.assertIn('"Search Terms": "Configure"', source)
        self.assertIn('"Exclusions": "Exclude"', source)
        self.assertIn('"Identify": "Identify"', source)
        self.assertIn('"Stats": "Track"', source)
        self.assertIn('"Upload Data": "Upload"', source)
        self.assertIn('"Get Started": "Learn"', source)
        self.assertIn('"outcomes": "Stats"', source)
        self.assertIn('"statistics": "Stats"', source)
        self.assertIn('st.markdown("## 🔎 Identify")', source)
        self.assertIn('st.markdown("## 📊 Track")', source)
        self.assertIn('st.markdown("## 📝 Configure")', source)
        self.assertIn('st.markdown("## 🚫 Exclude")', source)
        self.assertIn('st.markdown("## ✅ Learn")', source)
        ...
        self.assertIn("STATS_SUBTABS = [*STATS_PERIOD_FILTERED_SUBTABS]", source)
        ...
        self.assertIn("st.button(\n                stats_subtab_display_label(tab_name)", source)
        ...
        self.assertIn("stats-summary-value--fit", source)
        self.assertIn("font-size: clamp(1rem, 1.2vw, 2rem);", source)
        self.assertIn('label == "Top Team Member"', source)
        ...
        self.assertIn("max-width: min(27rem, calc(100vw - 2rem));", source)
        self.assertIn("min-width: min(15rem, calc(100vw - 2rem));", source)
        self.assertIn(".field-label .column-help", source)
